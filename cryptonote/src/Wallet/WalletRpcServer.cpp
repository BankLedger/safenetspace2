...
void wallet_rpc_server::processRequest(const CryptoNote::HttpRequest& request, CryptoNote::HttpResponse& response) {
  ...
  try {
	...
	//dnc1
	static std::unordered_map<std::string, JsonMemberMethod> s_methods_dnc1 = {
		{ "getbalance", makeMemberMethod(&wallet_rpc_server::on_dnc1_getbalance) },
		{ "getaddress", makeMemberMethod(&wallet_rpc_server::on_dnc1_getaddress) },
		{ "transfer", makeMemberMethod(&wallet_rpc_server::on_dnc1_transfer) },
		{ "store", makeMemberMethod(&wallet_rpc_server::on_dnc1_store) },
		{ "get_payments", makeMemberMethod(&wallet_rpc_server::on_dnc1_get_payments) },
		{ "get_incoming_tx", makeMemberMethod(&wallet_rpc_server::on_dnc1_get_incoming_tx) }
	};

	auto it = (jsonRequest.getDnc().compare("2.0")) ? s_methods_dnc1.find(jsonRequest.getMethod()) : s_methods.find(jsonRequest.getMethod());
	auto end = (jsonRequest.getDnc().compare("2.0")) ? s_methods_dnc1.end() : s_methods.end();
...
bool wallet_rpc_server::on_dnc1_getbalance(const wallet_rpc::COMMAND_RPC_GET_BALANCE_DNC1::request& req, wallet_rpc::COMMAND_RPC_GET_BALANCE_DNC1::response& res){
	auto locked_amount = m_wallet.pendingBalance();
	auto available_balance = m_wallet.actualBalance();
	res.balance = locked_amount + available_balance;
	res.unlocked_balance = available_balance;
	return true;
}
bool wallet_rpc_server::on_dnc1_transfer(const wallet_rpc::COMMAND_RPC_TRANSFER_DNC1::request& req, wallet_rpc::COMMAND_RPC_TRANSFER_DNC1::response& res){
	std::vector<CryptoNote::WalletLegacyTransfer> transfers;
	std::vector<CryptoNote::TransactionMessage> messages;
	for (auto it = req.destinations.begin(); it != req.destinations.end(); it++) {
		CryptoNote::WalletLegacyTransfer transfer;
...
bool wallet_rpc_server::on_dnc1_store(const wallet_rpc::COMMAND_RPC_STORE::request& req, wallet_rpc::COMMAND_RPC_STORE::response& res){
	return on_store(req,res);
}
bool wallet_rpc_server::on_dnc1_get_payments(const wallet_rpc::COMMAND_RPC_GET_PAYMENTS::request& req, wallet_rpc::COMMAND_RPC_GET_PAYMENTS::response& res){
	return on_get_payments(req,res);
}
bool wallet_rpc_server::on_dnc1_getaddress(const wallet_rpc::COMMAND_RPC_GET_ADDRESS_DNC1::request& req, wallet_rpc::COMMAND_RPC_GET_ADDRESS_DNC1::response& res) {
	try
	{
		...
		throw JsonRpc::JsonRpcError(WALLET_RPC_ERROR_CODE_UNKNOWN_ERROR, std::string("dnc1 Getaddress invalid size") + e.what());
	}
	return true;
}
bool wallet_rpc_server::on_dnc1_get_incoming_tx(const wallet_rpc::COMMAND_RPC_GET_INCOMING_TX_DNC1::request& req, wallet_rpc::COMMAND_RPC_GET_INCOMING_TX_DNC1::response& res){
  PaymentId expectedPaymentId;
  CryptoNote::BinaryArray payment_id_blob;

  if (!Common::fromHex(std::to_string(req.tx_start_index), payment_id_blob)) {
    throw JsonRpc::JsonRpcError(WALLET_RPC_ERROR_CODE_WRONG_PAYMENT_ID, "Payment ID has invald format");
  }
  ...
  res.tx_total_count = payments.size();

	Tools::wallet_rpc::incoming_tx_details in_tx;
	for (auto& wti : payments[0].transactions)
	{
		in_tx.amount = wti.totalAmount;
		in_tx.block_height = wti.blockHeight;
		in_tx.payment_id = wti.m_id;
		in_tx.tx_hash = reinterpret_cast<const char*>(&wti.hash);
		in_tx.unlock_time = wti.unlockTime;
