...
#define WALLET_RPC_STATUS_BUSY    "BUSY"
  ...
  struct COMMAND_RPC_GET_BALANCE_DNC1
  {
	  typedef CryptoNote::EMPTY_STRUCT request;
  ...
  struct COMMAND_RPC_TRANSFER_DNC1
  {
	  struct request
	  {
		  std::list<transfer_destination> destinations;
  ...
  struct COMMAND_RPC_GET_ADDRESS_DNC1 {
	  typedef CryptoNote::EMPTY_STRUCT request;
  ...
  struct COMMAND_RPC_GET_INCOMING_TX_DNC1 {
	  struct request
	  {
		  ...
		  void serialize(ISerializer& s) {
			  KV_MEMBER(tx_start_index)
			  KV_MEMBER(tx_get_count)
		  }
	  };
	  ...
	  struct response {
		  ...
		  void serialize(ISerializer& s) {
			  KV_MEMBER(incoming_txs)
			  KV_MEMBER(tx_total_count)
		  }
	  };
  };
}
