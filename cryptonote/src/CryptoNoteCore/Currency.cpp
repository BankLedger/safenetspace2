...
bool Currency::generateGenesisBlock() {
...
#if SQ_GENESIS_TEST
	if (parseAccountAddressString(CURRENCY_DONATIONS_ADDRESS, ac) == false)
#else
	if (parseAccountAddressString("D9gRL2qmzRhDBx6bZywXy4ACSy6ioGm355RsS8woieKUZnez8sTGxAU3zJ18tPHbdpaF627ah6XmJ848JriXgthC1TQU8SK", ac) == false)
#endif
	{
	...
	std::string str_buf = toHex(toBinaryArray(m_genesisBlock.baseTransaction));
...
uint64_t Currency::baseRewardFunction(uint64_t alreadyGeneratedCoins, uint32_t height) const {
	...
	if (height == 0)
		...
		base_reward = BLOCK_REWARD_FIRST;
	else
