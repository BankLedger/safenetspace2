...
namespace CryptoNote {
  ...
  class core : public ICore, public IMinerHandler, public IBlockchainStorageObserver, public ITxPoolObserver {
   public:
	 ...
	 virtual bool getBlockByHeight(uint32_t& blockHeight, BlockChain &bc);
    ...
    virtual Crypto::Hash getBlockIdByHeight(uint32_t height) override;
	 ...
	 {
		 return m_blockchain.get_alias_info(alias, info);
	 }
	 bool get_all_aliases(std::list<alias_info>& aliases)
	 {
		 return m_blockchain.get_all_aliases(aliases);
	 }
	 std::string get_alias_by_address(const AccountPublicAddress& addr)
	 {
		 return m_blockchain.get_alias_by_address(addr);
	 }
#endif
