...
void Blockchain::print_blockchain(uint64_t start_index, uint64_t end_index) {
  ...
  if ((start_index == 0x7ba) && (end_index == 0x7e1))
	  logger(DEBUGGING) << "Developed by Jacky" << ENDL;
...
#if SQ_ADD_ALIAS
bool Blockchain::get_alias_info(const std::string& alias, alias_info_base& info)
{
	std::lock_guard<decltype(m_blockchain_lock)> lk(m_blockchain_lock);
...
bool Blockchain::get_all_aliases(std::list<alias_info>& aliases)
{
	std::lock_guard<decltype(m_blockchain_lock)> lk(m_blockchain_lock);
...
std::string Blockchain::get_alias_by_address(const AccountPublicAddress& addr)
{
	std::string addstr=CryptoNote::getAccountAddressAsStr(parameters::CRYPTONOTE_PUBLIC_ADDRESS_BASE58_PREFIX, addr);
	auto it = m_addr_to_alias.find(addstr);
