...
struct COMMAND_RPC_GETBLOCKTEMPLATE_DNC1 {
	struct request {
		uint64_t reserve_size; //max 255 bytes
		std::string wallet_address;
		...
		void serialize(ISerializer &s) {
			KV_MEMBER(reserve_size)
				KV_MEMBER(wallet_address)
				KV_MEMBER(alias_details)
				//KV_MEMBER(dev_bounties_vote)
		}
	};
...
struct COMMAND_RPC_GET_LAST_BLOCK_HEADER_DNC1 {
...
struct COMMAND_RPC_GET_ALL_ALIASES
{
	...
	struct response
	{
		...
		void serialize(ISerializer &s) {
			//CryptoNote::serializeAsBinary(aliases, "aliases", s);
			KV_MEMBER(aliases)
			KV_MEMBER(status)
		}
	};
...
struct COMMAND_RPC_GET_BLOCKS_BY_HEIGHTS
{
	struct request
	{
		...
		void serialize(ISerializer &s) {
			KV_MEMBER(start_height)
			KV_MEMBER(blocks_count)
			KV_MEMBER(genesis_hash)
		}
	};
	...
	struct response
	{
		...
		void serialize(ISerializer &s) {
			KV_MEMBER(blocks)
			KV_MEMBER(miner_tx_hashs)
			KV_MEMBER(block_sizes)
			KV_MEMBER(miner_tx_sizes)
			KV_MEMBER(tx_sizes)
			KV_MEMBER(blocks_count)
			KV_MEMBER(status)
		}
