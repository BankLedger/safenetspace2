...
bool processServerAliasResponse(const std::string& response, std::string& address) {
  try {
    ...
    auto rootIt = json.getObject().find("darknetspacename1");
    ...
    auto dncIt = rootIt->second[0].getObject().find("dnc");
    if (dncIt == rootIt->second[0].getObject().end()) {
      return false;
    }

    address = dncIt->second.getString();
  } catch (std::exception&) {
    return false;
  }
...
#if SQ_ADD_BM
  m_consoleHandler.setHandler("create_group", boost::bind(&simple_wallet::create_group, this, _1), "Create message group");
  m_consoleHandler.setHandler("join_group", boost::bind(&simple_wallet::join_group, this, _1), "join_group <address> <key>");
  m_consoleHandler.setHandler("delete_group", boost::bind(&simple_wallet::delete_group, this, _1), "delete_group <group>");
  m_consoleHandler.setHandler("list_group", boost::bind(&simple_wallet::list_group, this, _1), "List message group");
  //m_consoleHandler.setHandler("name_group", boost::bind(&simple_wallet::name_group, this, _1), "name_group <group> <name>");
  m_consoleHandler.setHandler("send_message", boost::bind(&simple_wallet::send_message, this, _1), "send_message <group> [reply] <message>");
  m_consoleHandler.setHandler("list_message", boost::bind(&simple_wallet::list_message, this, _1), "list_message <group>");
  //m_consoleHandler.setHandler("reply_message", boost::bind(&simple_wallet::reply_message, this, _1), "reply_message <id> [reply] <message>");

  m_consoleHandler.setHandler("save_bm", boost::bind(&simple_wallet::save_bm, this, _1), "save version BM");
#endif
#if SQ_ADD_BM_SEGMENT
  m_consoleHandler.setHandler("segment", boost::bind(&simple_wallet::sendSegment, this, _1), "sigment");
#endif
#if SQ_ADD_ALIAS
  m_consoleHandler.setHandler("make_alias", boost::bind(&simple_wallet::make_alias, this, _1), "pay 101 DNC to miners to make an alias. make_alias <alias>  [comment] [viewkey]");
#endif
}
//----------------------------------------------------------------------------------------------------
...
#if SQ_ADD_BM2
  m_wallet_version = WALLET_VERSION_BM;
#else
  m_wallet_version = 2;
#endif
...
bool simple_wallet::new_wallet(const std::string &wallet_file, const std::string& password) {
  ...
  try {
    ...
    try {
#if SQ_ADD_BM2
		m_wallet->m_wallet_version = WALLET_VERSION_BM;
#endif
...
#if SQ_ADD_BM
bool simple_wallet::create_group(const std::vector<std::string> &args)
{
	AccountKeys mkeys;
	std::string str_add, str_vkey, str_skey;
	m_wallet->createBMGroup(mkeys);
	m_wallet->convertBMGroup(mkeys, str_add, str_vkey, str_skey);
	success_msg_writer() << "address: " << str_add << std::endl << "viewkey: " << str_vkey << std::endl << "spendkey: " << str_skey;

	return true;
}
//----------------------------------------------------------------------------------------------------
bool simple_wallet::join_group(const std::vector<std::string> &args)
{
	if (args.size() != 3)
	{
		logger(ERROR, BRIGHT_RED) << "parameters number wrong." ;
		return false;
	}

	int errcode = 0;
	std::string strname = "";
	if (!(m_wallet->addBMGroup(args[0], args[1], args[2], strname, errcode)))
	{
		switch (errcode)
		{
		case 1:
			logger(ERROR, BRIGHT_RED) << "Too many message groups.";
			return false;
		case 2:
			logger(ERROR, BRIGHT_RED) << "Wrong address.";
			return false;
		case 3:
			logger(ERROR, BRIGHT_RED) << "Wrong key.";
			return false;
		case 4:
			logger(ERROR, BRIGHT_RED) << "Group already exist.";
			return false;
		default:
			break;
		}
...
//----------------------------------------------------------------------------------------------------
bool simple_wallet::delete_group(const std::vector<std::string> &args)
{
	if (args.size() != 1)
	{
		logger(ERROR, BRIGHT_RED) << "parameters number wrong.";
	...
	int errcode = 0;
	if (!(m_wallet->deleteBMGroup(args[0], errcode)))
	{
		logger(ERROR, BRIGHT_RED) << "Delete failed.";
		return false;
	}
	return true;
}
//----------------------------------------------------------------------------------------------------
bool simple_wallet::list_group(const std::vector<std::string> &args)
{
	std::vector<std::string> vecadd;
	int errcode = 0;

	if (!(m_wallet->listBMGroup(vecadd, errcode)))
	{
		logger(ERROR, BRIGHT_RED) << "Delete failed.";
		return false;
	}

	for (auto &item : vecadd)
	{
		success_msg_writer() << item << std::endl;
	}
	return true;
}
//----------------------------------------------------------------------------------------------------
bool simple_wallet::name_group(const std::vector<std::string> &args)
{
	return true;
}
//----------------------------------------------------------------------------------------------------
bool simple_wallet::send_message(const std::vector<std::string> &args)
{
	if ((args.size() != 2) && (args.size() != 3))
		return false;
	if ((args.size() == 3) && (args[1] != "reply"))
		return false;

	AccountPublicAddress ignore;
	if (!m_currency.parseAccountAddressString(args[0], ignore))
		throw std::runtime_error("Address \"" + args[0] + "\" is invalid");

	std::vector<WalletLegacyTransfer> m_tran;
	WalletLegacyTransfer tran;
	tran.address = args[0];
	tran.amount = CryptoNote::parameters::MINIMUM_FEE;
	m_tran.emplace_back(tran);

	std::vector<TransactionMessage> messages;
	TransactionMessage message;
	message.address = args[0];
	message.m_wallet_version = WALLET_VERSION_BM;
	message.m_id = 0;
	if (args.size() == 3)
		message.message = "Reply-To: " + m_wallet->getAddress() + "\n\n" + args[2];
	else
	...
	try
	{
		...
		CryptoNote::TransactionId tx = m_wallet->sendTransaction(m_tran, CryptoNote::BMESSAGE_FEE, "", 0, 0, messages);
		if (tx == WALLET_LEGACY_INVALID_TRANSACTION_ID)
		{
			fail_msg_writer() << "Can't send money";
			return true;
		...
		CryptoNote::WalletLegacyTransaction txInfo;
		m_wallet->getTransaction(tx, txInfo);
		success_msg_writer(true) << "Message successfully sent, transaction " << Common::podToHex(txInfo.hash);
...
//----------------------------------------------------------------------------------------------------
bool simple_wallet::reply_message(const std::vector<std::string> &args)
{
	if ((args.size() != 2) && (args.size() != 3))
		return false;
	if ((args.size() == 3) && (args[1] != "reply"))
	...
	message.m_id = 0;
	if (args.size() == 3)
		message.message = "Reply-To: " + args[0] + "\n\n" + args[2];
	else
		message.message = args[1];
	messages.emplace_back(message);
...
//----------------------------------------------------------------------------------------------------
bool simple_wallet::list_message(const std::vector<std::string> &args)
{
	if (args.size() != 1)
	{
		logger(ERROR, BRIGHT_RED) << "parameters number wrong.";
	...
	std::vector<CryptoNote::WalletLegacyTransaction> tranVec;
	int errcode = 0;

	if (!(m_wallet->getBMMessage(args[0],  tranVec, errcode)))
	{
		switch (errcode)
		{
		case 1:
			logger(ERROR, BRIGHT_RED) << "No such address.";
			return false;
		case 2:
			logger(ERROR, BRIGHT_RED) << "Wrong address.";
			return false;
	...
	size_t len = tranVec.size();
	for (size_t i = 0; i < len; i ++)
	{
		CryptoNote::WalletLegacyTransaction tran = tranVec[i];
		if (tran.messages.size() > 0)
		{
			//if (tran.messages[0].size() > 1)
			{
				char timeString[TIMESTAMP_MAX_WIDTH + 1];
				time_t timestamp = static_cast<time_t>(tran.timestamp);
				if (std::strftime(timeString, sizeof(timeString), "%Y-%m-%d %H:%M:%S", std::gmtime(&timestamp)) == 0)
				{
					throw std::runtime_error("time buffer is too small");
				}
				std::string straddr, strmsg;
				if ((tran.messages[0].size() < 104) || (tran.messages[0].find_first_of(strMSG) != 0))
				{
					strmsg = tran.messages[0];
				}
				else
				{
					int po = tran.messages[0].find_first_of('\n');
					straddr = tran.messages[0];
					straddr.resize(po);
					straddr.erase(0, 10);
				...
				//success_msg_writer() << " " << timeString << " " << strmsg << " " << straddr;
				success_msg_writer() << i << " " << "\n" << strmsg << "\n" << straddr << "\n";
			}
		}
	}
	return true;
...
//----------------------------------------------------------------------------------------------------
bool simple_wallet::save_bm(const std::vector<std::string> &args)
{
	m_wallet->m_wallet_version = WALLET_VERSION_BM;
	try
	{
		CryptoNote::WalletHelper::storeWallet(*m_wallet, m_wallet_file);
		success_msg_writer() << "Wallet_v3 data saved";
	}
	catch (const std::exception& e)
	{
		fail_msg_writer() << e.what();
...
#if SQ_ADD_BM_SEGMENT
//----------------------------------------------------------------------------------------------------
bool simple_wallet::sendSegment(const std::vector<std::string> &args)
{
	int size = CryptoNote::SEGMENT_SIZE;
	CryptoNote::TransactionId tx;
	m_wallet->sendSegment(size, tx);
	if ((size < 1) || (tx == WALLET_LEGACY_INVALID_TRANSACTION_ID))
	{
		fail_msg_writer() << "segment failed";
	}
	else
	{
		success_msg_writer() << "segment successful";
	}
	return true;
}
#endif
...
bool simple_wallet::get_daemon_alias(const std::string& alias, std::string& err)
{
	...
	req.alias = alias;
	COMMAND_RPC_GET_ALIAS_DETAILS::response res;
	err.clear();

	try
	{
		HttpClient httpClient(m_dispatcher, m_daemon_host, m_daemon_port); 
		//JsonRpc::invokeJsonRpcCommand(httpClient, "getaliasdetails", req, res);
		JsonRpc::invokeJsonRpcCommand(httpClient, "getblockheaderbyheight", req, res);
		if (res.status != CORE_RPC_STATUS_OK)
		{
			err = "Core responded with wrong status: " + res.status;
			return true;
		}
	}
	catch (std::exception& e)
	{
		err = "Failed to get daemon alias: " + string(e.what());
		return true;
	}
#if 0
	try {
		...
		invokeJsonCommand(httpClient, "/getaliasdetails", req, res);
		err = interpret_rpc_response(true, res.status);
		if (!(err.empty()))
			fail_msg_writer()  << err;
	}
	catch (const ConnectException&) {
...
bool simple_wallet::make_alias(const std::vector<std::string> &args)
{
	...
	{
		fail_msg_writer() << "usage: make_alias <alias>  [comment] [viewkey]";
		return true;
	}
	std::string str;
	if (m_node->getLastLocalBlockHeight() < CURRENCY_BLOCK_GRANTED_FULL_REWARD_ZONE_ENLARGE_STARTING_BLOCK)
	{
#if 0
...
#else
		str = "Alias name can't be make untill the 147000th block reached.";
#endif
		fail_msg_writer() << str;
		return true;
	}
	...
	std::string errbuf;
	if (!CryptoNote::validate_alias_name(str, errbuf))
	{
		fail_msg_writer() << "Wrong alias name:" << errbuf;
		return true;
	}
	...
	if (get_daemon_alias(str, err))
	{
		if (err == CORE_RPC_STATUS_OK)
			fail_msg_writer() << "Alias already exists";
		else if (err == "")
			fail_msg_writer() << "No connection to daemon";
		else
			fail_msg_writer() << err;
		return true;
	}

	CryptoNote::alias_info ai = boost::value_initialized<CryptoNote::alias_info>();
	ai.m_alias = str;

	uint64_t prefix;
	std::string strAddr;
	if (!m_wallet->getOwnAddress(strAddr))
	{
		fail_msg_writer() << "can not find own address.";
		return false;
	}
	if (!CryptoNote::parseAccountAddressString(prefix, ai.m_address, strAddr))
	{
		fail_msg_writer() << "target account address has wrong format.";
		return false;
	}
	if (prefix != parameters::CRYPTONOTE_PUBLIC_ADDRESS_BASE58_PREFIX)
	{
		fail_msg_writer() << "Wrong address prefix: " << prefix << ", expected " << parameters::CRYPTONOTE_PUBLIC_ADDRESS_BASE58_PREFIX;
		return false;
	}
	...
	if (args.size() >= 2)
	{
		if (args[1].size() > 64)
		{
			fail_msg_writer() << "comment too long, must be less 64 chars.";
			return true;
		}
	...
	if (args.size() == 3)
	{
		...
		std::string bin_str;
        Common::parse_hexstr_to_binbuff(str, bin_str);
		ai.m_view_key = *reinterpret_cast<const Crypto::SecretKey*>(bin_str.c_str());
	}

	std::string buff;
	bool r = make_tx_extra_alias_entry(buff, ai, err);
	if (!r)
		return false;
	CryptoNote::WalletLegacyTransfer transfer;
	transfer.address = CURRENCY_DONATIONS_ADDRESS; 
	transfer.amount = parameters::MINIMUM_FEE;
	transfer.m_id = ACCOUNT_OWN_ID;
	transfer.m_wallet_version = WALLET_VERSION_BM;
	std::vector<TransactionMessage> messages;
	...
	try
	{
		...
		CryptoNote::TransactionId tx = m_wallet->sendTransaction(transfer, MAKE_ALIAS_MINIMUM_FEE, buff, 0, 0, messages);
		if (tx == WALLET_LEGACY_INVALID_TRANSACTION_ID)
		{
			fail_msg_writer() << "Can't make alias.";
			return true;
		}
		...
		CryptoNote::WalletLegacyTransaction txInfo;
		m_wallet->getTransaction(tx, txInfo);
		success_msg_writer(true) << "Alias successfully made, transaction " << Common::podToHex(txInfo.hash);
