...
public:
  ...
  bool getTransactions(const uint8_t& id, std::vector<CryptoNote::WalletLegacyTransaction>& tranVec, int &errcode) const;
  std::deque<std::unique_ptr<WalletLegacyEvent>> onTransactionUpdated(const TransactionInformation& txInfo);
#endif

#if SQ_ADD_BM_WALLET
  bool getTransaction(TransactionId groupId, TransactionId transactionId, WalletLegacyTransaction& transaction) const;
  size_t getTransactionCount(CryptoNote::TransactionId _groupId) const;
  bool deleteTransactionById(const int& _id);
#endif
...
private:
  ...
  UserTransactions m_transactionsBM;
};
