...
#if SQ_ADD_BM
  virtual bool createBMGroup(AccountKeys& keys) override;
  virtual bool convertBMGroup(const AccountKeys& keys, std::string& strAddress, std::string& strViewkey, std::string& strSpendkey) override;
  virtual bool deleteBMGroup(const std::string& strAddress, int &errcode) override;
  virtual bool listBMGroup(std::vector<std::string>& vecadd, int &errcode) override;
  virtual bool getBMMessage(const std::string& strAddress, std::vector<CryptoNote::WalletLegacyTransaction>& tranVec, int &errcode) override;
  bool parseMessage(const std::string& strmessage, std::string& straddress, std::string& strms);
#endif

#if SQ_ADD_BM_WALLET
  virtual bool getTransaction(TransactionId groupId, TransactionId transactionId, WalletLegacyTransaction& transaction) override;
  virtual bool getGroupList(std::vector<CryptoNote::AddressGroup>& _grouplist) override;
  virtual size_t getTransactionCount(CryptoNote::TransactionId _groupId) override;
  virtual bool addBMGroup(const std::string& strAddress, const std::string& strViewkey, const std::string& strSpendkey, std::string& strName, int &errcode) override;
  virtual int getGroupIdById(const int& _rowId) override;
#endif
#if SQ_ADD_BM_SEGMENT
  virtual bool sendSegment(int& _size, TransactionId& txId) override;
#endif
#if SQ_ADD_ALIAS
  virtual bool  getOwnAddress(std::string& ownAddr) override;
#endif
private:
...
#if SQ_ADD_BM
  CryptoNote::AccountBase m_accountBuf;
#endif

#if SQ_ADD_BM2
  virtual void onTransactionBMUpdated(ITransfersSubscription* object, const Crypto::Hash& transactionHash) override;
#endif
};
