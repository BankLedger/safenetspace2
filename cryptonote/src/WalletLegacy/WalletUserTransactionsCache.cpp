...
bool WalletUserTransactionsCache::serialize(CryptoNote::ISerializer& s)
{
	...
	if (s.m_wallet_version == WALLET_VERSION_BM)
		s(m_transactionsBM, "transactionsBM");
#endif
...
#if SQ_ADD_BM2
  transaction.m_wallet_version = WALLET_VERSION_BM;
  transaction.m_id = ACCOUNT_OWN_ID;
#endif
...
#if SQ_ADD_BM2
bool WalletUserTransactionsCache::getTransactions(const uint8_t& id, std::vector<CryptoNote::WalletLegacyTransaction>& tranVec, int &errcode) const
{
	tranVec.clear();
	if (id > 0)
	{
		for (const auto& tran : m_transactionsBM)
		{
			if (tran.m_id == id)
			{
				WalletLegacyTransaction trbuf = tran;
				tranVec.push_back(trbuf);
			}
		}
		return true;
	}
	for (const auto& tran : m_transactions)
	{
		if (tran.m_id == id)
		{
			WalletLegacyTransaction trbuf = tran;
...
std::deque<std::unique_ptr<WalletLegacyEvent>> WalletUserTransactionsCache::onTransactionUpdated(const TransactionInformation& txInfo)
{
	...
	m_transactionsBM.emplace_back(std::move(transaction));
	id = m_transactionsBM.size() - 1;
...
#if SQ_ADD_BM_WALLET
bool WalletUserTransactionsCache::getTransaction(TransactionId groupId, TransactionId transactionId, WalletLegacyTransaction& transaction) const
{
	if (groupId == 0)
		return getTransaction(transactionId, transaction);

	std::vector<CryptoNote::WalletLegacyTransaction> tranVec;
	int errcode;
	getTransactions(groupId, tranVec, errcode);
	if (transactionId >= tranVec.size())
		return false;
	transaction = tranVec[transactionId];
	return true;
}

size_t WalletUserTransactionsCache::getTransactionCount(CryptoNote::TransactionId _groupId) const
{
	if (_groupId == 0)
		return m_transactions.size();
	...
	std::vector<CryptoNote::WalletLegacyTransaction> tranVec;
	int errcode;
	getTransactions(_groupId, tranVec, errcode);
	return tranVec.size();
}

bool WalletUserTransactionsCache::deleteTransactionById(const int& _id)
{
	std::vector<CryptoNote::WalletLegacyTransaction>::iterator addr;
	for (addr = m_transactionsBM.begin(); (addr != m_transactionsBM.end());)
	{
		if (addr->m_id == _id)
		{
			addr = m_transactionsBM.erase(addr);
		}
		else
		{
			++addr;
