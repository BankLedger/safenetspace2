...
void WalletLegacy::initSync() {
#if SQ_ADD_BM2
	std::vector<AccountSubscription> subBM;
	size_t len = m_account.get_BMSize();
	for (size_t i = 0; i < len; i++)
	{
		AccountSubscription sub;
		sub.keys = reinterpret_cast<const AccountKeys&>(m_account.getAccountKeyById(i));
		sub.transactionSpendableAge = SPEND_AGE;
	...
	if (len == 0)
	{
		...
		sub.syncStart.height = 0;
		sub.syncStart.timestamp = m_account.get_createtime() - ACCOUN_CREATE_TIME_ACCURACY;
		sub.keys.m_id = ACCOUNT_OWN_ID;
		sub.keys.m_name = "group_0";

		subBM.push_back(sub);
		m_account.addAccountKeys(sub.keys);
	}
	auto& subObject = m_transfersSync.addSubscription(subBM);
#else
  AccountSubscription sub;
  sub.keys = reinterpret_cast<const AccountKeys&>(m_account.getAccountKeys());
  sub.transactionSpendableAge = 3;
  sub.syncStart.height = 0;
  sub.syncStart.timestamp = m_account.get_createtime() - ACCOUN_CREATE_TIME_ACCURACY;
...
void WalletLegacy::doLoad(std::istream& source) {
  ...
  try {
	...
	this->m_wallet_version = serializer.m_wallet_version;
    initSync();
...
#if SQ_ADD_BM2 
		m_transfersSync.m_wallet_version = m_wallet_version;
#endif
...
#if SQ_ADD_BM2
	this->m_transactionsCache.m_wallet_version = m_wallet_version;
	if (m_wallet_version == WALLET_VERSION_BM)
		serializer.serializeBM(destination, m_password, saveDetailed, cache);
	else
...
#if SQ_ADD_BM2
bool WalletLegacy::createBMGroup(AccountKeys& keys)
{
	m_accountBuf.generate();
	keys = m_accountBuf.getAccountKeys();
	return true;
}

bool WalletLegacy::convertBMGroup(const AccountKeys& keys, std::string& strAddress, std::string& strViewkey, std::string& strSpendkey)
{
	strAddress = m_currency.accountAddressAsString(keys.address);
	strViewkey = Common::podToHex(keys.viewSecretKey);
	strSpendkey = Common::podToHex(keys.spendSecretKey);
	return true;
}

bool WalletLegacy::deleteBMGroup(const std::string& strAddress, int &errcode)
{
	int id = 0;
	bool re = m_account.deleteBMGroup(strAddress, id, errcode);
	if (re)
		re = m_transfersSync.deleteKeyBM(id);
	if (re)
		re = m_transactionsCache.deleteTransactionById(id);
	return re;
}

bool WalletLegacy::listBMGroup(std::vector<std::string>& vecadd, int &errcode)
{
	return m_account.listBMGroup(vecadd, errcode);
}

bool WalletLegacy::getBMMessage(const std::string& strAddress, std::vector<CryptoNote::WalletLegacyTransaction>& tranVec, int &errcode)
{
	AccountKeys m_keys;
	uint8_t id;
	if (!(m_currency.parseAccountAddressString(strAddress, m_keys.address)))
	{
		errcode = 2;
		return false;
	}
	if (!(m_account.checkAddrExist(strAddress, id)))
	{
		errcode = 1;
		return false;
	}
	...
	return m_transactionsCache.getTransactions(id, tranVec, errcode);
}

void WalletLegacy::onTransactionBMUpdated(ITransfersSubscription* object, const Crypto::Hash& transactionHash)
{
	std::deque<std::unique_ptr<WalletLegacyEvent>> events;

	TransactionInformation txInfo;
	if (m_transferDetails->getTransactionInformationBM(transactionHash, txInfo))
	{
		std::unique_lock<std::mutex> lock(m_cacheMutex);
		events = m_transactionsCache.onTransactionUpdated(txInfo);
	}

	//notifyClients(events);
}

bool WalletLegacy::parseMessage(const std::string& strmessage, std::string& straddress, std::string& strms)
{
	straddress.clear();
	strms.clear();

	if ((strmessage.size() < 104) || (strmessage.find_first_of(strMSG) != 0))
	{
		strms = strmessage;
		return true;
	}

	int po = strmessage.find_first_of('\n');
	straddress = strmessage;
	straddress.resize(po);
	straddress.erase(0, 10);
...
#if SQ_ADD_BM_WALLET
bool WalletLegacy::getTransaction(TransactionId groupId, TransactionId transactionId, WalletLegacyTransaction& transaction)
{
	std::unique_lock<std::mutex> lock(m_cacheMutex);
	throwIfNotInitialised();
	return m_transactionsCache.getTransaction(groupId, transactionId, transaction);
}

bool WalletLegacy::getGroupList(std::vector<CryptoNote::AddressGroup>& _grouplist)
{
	std::unique_lock<std::mutex> lock(m_cacheMutex);
	throwIfNotInitialised();	
	return m_account.getGroupList(_grouplist);
}

size_t WalletLegacy::getTransactionCount(CryptoNote::TransactionId _groupId)
{
	std::unique_lock<std::mutex> lock(m_cacheMutex);
	throwIfNotInitialised();
	return m_transactionsCache.getTransactionCount(_groupId);
}

bool WalletLegacy::addBMGroup(const std::string& strAddress, const std::string& strViewkey, const std::string& strSpendkey, std::string& strName, int &errcode)
{
	std::unique_lock<std::mutex> lock(m_cacheMutex);
	throwIfNotInitialised();
	errcode = 0;
	...
	if (!Common::podFromHex(strViewkey, m_keys.viewSecretKey))
	{
		errcode = 3;
		return false;
	}
	if (!Common::podFromHex(strSpendkey, m_keys.spendSecretKey))
	{
		errcode = 3;
		return false;
	}
#if SQ_ADD_BM2
	bool re = m_account.addBMGroup(strAddress, m_keys, strName, errcode);
	if (re)
		re = m_transfersSync.addKeyBM(m_keys);
...
int WalletLegacy::getGroupIdById(const int& _rowId)
{
	std::unique_lock<std::mutex> lock(m_cacheMutex);
	throwIfNotInitialised();
	return m_account.getGroupIdById(_rowId);
}
#endif
#if SQ_ADD_BM_SEGMENT
bool WalletLegacy::sendSegment(int& _size, TransactionId& txId)
{
	uint64_t _fee = CryptoNote::parameters::MINIMUM_FEE;
	uint64_t segFee = CryptoNote::parameters::MINIMUM_FEE + CryptoNote::BMESSAGE_FEE;
	int size = (actualBalance() - _fee) / segFee;
	if (size < _size)
		_size = size;
	if (_size < 1)
		return false;
	...
	std::string strAddr;
	m_account.getOwnAddress(strAddr);
	for (int i = 0; i < _size; i++)
	{
		WalletLegacyTransfer tran;
		tran.address = strAddr;
		tran.amount = segFee;
#if SQ_ADD_BM2
		tran.m_id = ACCOUNT_OWN_ID;
		tran.m_wallet_version = WALLET_VERSION_BM;
#endif
		m_tran.emplace_back(tran);
	}
	std::vector<TransactionMessage> _messages;
	txId = sendTransaction(m_tran, _fee, "", 0, 0, _messages);
...
#if SQ_ADD_ALIAS
bool WalletLegacy::getOwnAddress(std::string& ownAddr)
{
	return m_account.getOwnAddress(ownAddr);
}
#endif
} //namespace CryptoNote
