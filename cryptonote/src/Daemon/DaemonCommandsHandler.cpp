...
//#if SQ_ADD_ALIAS
#if 0
  m_consoleHandler.setHandler("alias", boost::bind(&DaemonCommandsHandler::alias, this, _1), "Print all alias");
  m_consoleHandler.setHandler("make_alias", boost::bind(&DaemonCommandsHandler::make_alias, this, _1), "Puts alias reservation record into block template, if alias is free");
#endif
}
...
//#if SQ_ADD_ALIAS
#if 0
bool DaemonCommandsHandler::make_alias(const std::vector<std::string>& args)
{
	if (args.size() != 2)
	{
	...
	if (!CryptoNote::validate_alias_name(args[0]))
	{
	...
	if (!CryptoNote::get_account_address_from_str(ai.m_address, args[1]))
	{
		std::cout << "target account address has wrong format" << std::endl;
		return true;
	}
...
bool DaemonCommandsHandler::alias(const std::vector<std::string>& args)
{
	...
	for (auto a : aliases)
	{
		...
		std::cout << "address: " << CryptoNote::get_account_address_as_str(a.m_address) << ENDL;

		if (a.m_text_comment.size())
			std::cout << "comment: " << a.m_text_comment << ENDL;

		if (a.m_view_key != CryptoNote::NULL_SECRET_KEY)
			std::cout << "viewkey: " << Common::pod_to_hex(a.m_view_key) << ENDL;
