...
#if SQ_ADD_BM2
  if (s.m_wallet_version == WALLET_VERSION_BM)
  {
	  s(ti.m_id, "");
  }
#endif
...
#if SQ_ADD_BM2
	  if (transfer.m_id != ACCOUNT_OWN_ID)
		  continue;
#endif
...
#if SQ_ADD_BM2
  s.m_wallet_version = m_wallet_version;
  if (m_wallet_version == WALLET_VERSION_BM)
	  writeSequence<TransactionInformation>(m_transactionsBM.begin(), m_transactionsBM.end(), "transactionsBM", s);
#endif
...
void TransfersContainer::load(std::istream& in) {
...
#if SQ_ADD_BM2
  TransactionMultiIndex transactionsBM;
#endif
  ...
  s.m_wallet_version = m_wallet_version;
  if (m_wallet_version == WALLET_VERSION_BM)
	  readSequence<TransactionInformation>(std::inserter(transactionsBM, transactionsBM.end()), "transactionsBM", s);
#endif
...
#if SQ_ADD_BM2
  m_transactionsBM = std::move(transactionsBM);
#endif
...
#if SQ_ADD_BM2
bool TransfersContainer::addTransaction(const TransactionBlockInfo& block, const ITransactionReader& tx,	const std::vector<TransactionOutputInformationIn>& transfers,
	std::vector<std::string>&& messages, const uint8_t& id, std::vector<TransactionOutputInformation>* unlockingTransfers)
{
	std::unique_lock<std::mutex> lock(m_mutex);
	...
	if (added) {
		addTransaction(block, tx, std::move(messages), id);
	}
...
void TransfersContainer::addTransaction(const TransactionBlockInfo& block, const ITransactionReader& tx, std::vector<std::string>&& messages, const uint8_t& m_id)
{
	auto txHash = tx.getTransactionHash();
...
bool TransfersContainer::addTransactionBM(const TransactionBlockInfo& block, const ITransactionReader& tx, std::vector<std::string>&& messages, const uint8_t& id)
{
	std::unique_lock<std::mutex> lock(m_mutex);
	...
	if (m_transactionsBM.count(tx.getTransactionHash()) > 0)
	{
		return false;
		//throw std::invalid_argument("Transaction is already added");
	}
	...
	txInfo.totalAmountOut = 0;
	txInfo.extra = tx.getExtra();
	txInfo.messages = std::move(messages);
	txInfo.m_id = id;	
	txInfo.paymentId = NULL_HASH;

	auto result = m_transactionsBM.emplace(std::move(txInfo));
	(void)result; // Disable unused warning
	assert(result.second);
...
bool TransfersContainer::getTransactionInformationBM(const Crypto::Hash& transactionHash, TransactionInformation& info) const
{
	std::lock_guard<std::mutex> lk(m_mutex);
	auto it = m_transactionsBM.find(transactionHash);
	if (it == m_transactionsBM.end())
		return false;
