...
#if SQ_ADD_BM2
bool TransfersConsumer::addViewScret(Crypto::SecretKeyBM& viewSecret)
{
	m_viewSecretBM.push_back(viewSecret);
	return true;
}

bool TransfersConsumer::deleteViewScret(const int& _id)
{
	std::vector<Crypto::SecretKeyBM>::iterator addr;
	for (addr = m_viewSecretBM.begin(); addr != m_viewSecretBM.end(); )
	{
		if (addr->m_id == _id)
		{
			addr = m_viewSecretBM.erase(addr);
			return true;
		}
		else
			++addr;
...
#if SQ_ADD_BM2
std::error_code createTransfers(const AccountKeys& account, const TransactionBlockInfo& blockInfo, const ITransactionReader& tx, const std::vector<uint32_t>& outputs,
	const std::vector<uint32_t>& globalIdxs, const uint8_t& id, std::vector<TransactionOutputInformationIn>& transfers)
{
	auto txPubKey = tx.getTransactionPublicKey();
...
{
#if SQ_ADD_BM2
	size_t len = m_viewSecretBM.size();
	for (size_t i = 0; i < len; i ++)
	{
		std::unordered_map<PublicKey, std::vector<uint32_t>> outputs;
		Crypto::SecretKeyBM skey = m_viewSecretBM[i];
		std::unordered_set<Crypto::PublicKey> spendkey;
		spendkey.insert(skey.m_spendPubKey);
		findMyOutputs(tx, skey.m_viewSecretkey, spendkey, outputs);
		...
		if (i == 0)
		{
			...
			for (const auto& kv : outputs)
			{
				...
				if (it != m_subscriptions.end())
				{
					...
					errorCode = createTransfers(it->second->getKeys(), blockInfo, tx, kv.second, info.globalIdxs, skey.m_id, transfers);
					if (errorCode)
					{
						brun = true;
						continue;
		...
		else
		{
			...
			for (const auto& kv : outputs)
			{
				...
				{
					find = 1;
					auto messages = get_messages_from_extra(tx.getExtra(), tx.getTransactionPublicKey(), &skey.m_spendSecretKey);
					bool added = it->second->addTransaction(blockInfo, tx, std::move(messages), skey.m_id);
				}
			}
		}
	}
...
#if SQ_ADD_BM2
	processOutputs(blockInfo, *kv.second, tx, subscriptionOutputs, info.globalIdxs, containerContainsTx, containerUpdated, info.m_id);
#else
	...
	const std::vector<TransactionOutputInformationIn>& transfers, const std::vector<uint32_t>& globalIdxs, bool& contains, bool& updated, uint8_t m_id)
{
	...
	{
		auto messages = get_messages_from_extra(tx.getExtra(), tx.getTransactionPublicKey(), &sub.getKeys().spendSecretKey);
		updated = sub.addTransaction(blockInfo, tx, transfers, std::move(messages), m_id);
		contains = updated;
	}
}
#endif
