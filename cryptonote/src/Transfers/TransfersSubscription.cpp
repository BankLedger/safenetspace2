	...
	const std::vector<TransactionOutputInformationIn>& transfersList,	std::vector<std::string>&& messages, const uint8_t& id)
{
	...
	bool added = transfers.addTransaction(blockInfo, tx, transfersList, std::move(messages), id, &unlockedTransfers);
	if (added)
	{
		m_observerManager.notify(&ITransfersObserver::onTransactionUpdated, this, tx.getTransactionHash());
	}
...
bool TransfersSubscription::addTransaction(const TransactionBlockInfo& blockInfo, const ITransactionReader& tx, std::vector<std::string>&& messages, const uint8_t& id)
{
	bool added = transfers.addTransactionBM(blockInfo,  tx, std::move(messages), id);
	if (added)
	{
		m_observerManager.notify(&ITransfersObserver::onTransactionBMUpdated, this, tx.getTransactionHash());
	}
