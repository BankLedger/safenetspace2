  ...
  for (const auto& consumer : m_consumers) {
    ...
    for (auto& addr : subscriptions) {
      ...
      if (sub != nullptr) {
        ...
        assert(sub);
		sub->getContainer().m_wallet_version = m_wallet_version;
    ...
    while (subscriptionCount--) {
      ...
      if (subIter != m_consumers.end()) {
        ...
        while (subCount--) {
          ...
          if (sub != nullptr) {
#if SQ_ADD_BM2
			  sub->getContainer().m_wallet_version = this->m_wallet_version;
#endif
...
#if SQ_ADD_BM2
		  consumer->getSubscription(sub.first)->getContainer().m_wallet_version = m_wallet_version;
#endif
...
#if SQ_ADD_BM2
ITransfersSubscription& TransfersSyncronizer::addSubscription(const std::vector<AccountSubscription>& acc)
{
	auto it = m_consumers.find(acc[0].keys.address.viewPublicKey);

	if (it == m_consumers.end())
	{
		std::unique_ptr<TransfersConsumer> consumer(new TransfersConsumer(m_currency, m_node, acc[0].keys.viewSecretKey));
		for (auto& sub : acc)
		{
			SecretKeyBM SKBM;
			SKBM.m_viewSecretkey = sub.keys.viewSecretKey;
			SKBM.m_spendSecretKey = sub.keys.spendSecretKey;
			SKBM.m_spendPubKey = sub.keys.address.spendPublicKey;
			SKBM.m_id = sub.keys.m_id;
			consumer.get()->addViewScret(SKBM);
		}

		m_sync.addConsumer(consumer.get());
		consumer->addObserver(this);
		it = m_consumers.insert(std::make_pair(acc[0].keys.address.viewPublicKey, std::move(consumer))).first;
	}

	return it->second->addSubscription(acc[0]);
}

bool TransfersSyncronizer::addKeyBM(const CryptoNote::AccountKeys& keys)
{
	if (m_consumers.size() < 1)
		return false;

	SecretKeyBM SKBM;
	SKBM.m_viewSecretkey = keys.viewSecretKey;
	SKBM.m_spendSecretKey = keys.spendSecretKey;
	SKBM.m_spendPubKey = keys.address.spendPublicKey;
	SKBM.m_id = keys.m_id;

	return m_consumers.begin()->second->addViewScret(SKBM);
}

bool TransfersSyncronizer::deleteKeyBM(const int& _id)
{
	if (m_consumers.size() < 1)
		return false;
	return m_consumers.begin()->second->deleteViewScret(_id);
}
#endif
}
