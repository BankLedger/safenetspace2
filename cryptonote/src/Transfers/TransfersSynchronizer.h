...
public:
  ...
  ITransfersSubscription& addSubscription(const std::vector<AccountSubscription>& acc);
  bool addKeyBM(const CryptoNote::AccountKeys& keys);
  bool deleteKeyBM(const int& _id);
#endif
