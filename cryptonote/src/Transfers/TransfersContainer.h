...
public:
  ...
  bool addTransaction(const TransactionBlockInfo& block, const ITransactionReader& tx,	  const std::vector<TransactionOutputInformationIn>& transfers,
	  std::vector<std::string>&& messages, const uint8_t& id, std::vector<TransactionOutputInformation>* unlockingTransfers = nullptr);
  bool addTransactionBM(const TransactionBlockInfo& block, const ITransactionReader& tx, std::vector<std::string>&& messages, const uint8_t& id);
  virtual bool getTransactionInformationBM(const Crypto::Hash& transactionHash, TransactionInformation& info) const override;
#endif
...
#if SQ_ADD_BM2
  TransactionMultiIndex m_transactionsBM;
  void addTransaction(const TransactionBlockInfo& block, const ITransactionReader& tx, std::vector<std::string>&& messages, const uint8_t& m_id);
#endif
};
