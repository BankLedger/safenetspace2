...
public:
#if SQ_ADD_BM_WALLET
	enum Columns { COLUMN_LABEL = 0, COLUMN_ADDRESS, COLUMN_TYPE, COLUMN_NO };
	enum Roles { ROLE_LABEL = Qt::UserRole, ROLE_ADDRESS, ROLE_TYPE, ROLE_NO };
#else
...
#if SQ_ADD_BM_WALLET
  void addAddress(const QString& _label, const QString& _address, int _type = 0, int _no = 0);
#else
