...
public:
  ...
  void convertWallet(const QString& _filepath, const QString& _password);
  ...
  void setOldWalletFile(const QString& _path);
...
#if SQ_ADD_BM_WALLET
  bool getTransaction(CryptoNote::TransactionId _groupId, CryptoNote::TransactionId _id, CryptoNote::WalletLegacyTransaction& _transaction);
  bool getGroupList(std::vector<CryptoNote::AddressGroup>& _grouplist);
  quint64 getTransactionCount(CryptoNote::TransactionId _groupId) const;
  bool addBMGroup(const std::string& strAddress, const std::string& strViewkey, const std::string& strSpendkey, std::string strName="");
  bool deleteBMGroup(const std::string& strAddress);
  bool createGroup(std::string& str_add, std::string& str_vkey, std::string& str_skey);
  int getGroupIdById(const int& _rowId);
#endif
#if SQ_ADD_BM_SEGMENT
  void sendSegment(int& _size);
#endif

private:
  std::fstream m_file;
  CryptoNote::IWalletLegacy* m_wallet;
  CryptoNote::IWalletLegacy* m_wallet_old;
  QString  m_walletfile_old;
  QMutex m_mutex;
...
#if SQ_ADD_BM_WALLET
  void addMessageGroupSignal();
  void deleteMessageGroupSignal();
#endif
};
}
