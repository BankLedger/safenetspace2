  ...
  void start(Node** _node, const CryptoNote::Currency* currency,  INodeCallback* _callback, Logging::LoggerManager* _loggerManager,
    ...
    try {
      (*_node)->init([this](std::error_code _err) {
		  ...
		  Q_EMIT NodeAdapter::instance().nodeInitCompletedSignal1();
          QCoreApplication::processEvents();
        });
...
bool NodeAdapter::getBlockByHeight(uint32_t& blockHeight, CryptoNote::BlockChain &bc)
{
	Q_CHECK_PTR(m_node);
	return m_node->getBlockByHeight(blockHeight, bc);
}

#if SQ_DOGETBLOCK_TEST
INode*NodeAdapter:: getnode()
{
	Q_CHECK_PTR(m_node);
	return m_node->getnode();
}

bool NodeAdapter::doGetBlocks(const std::vector<uint32_t>& blockHeights, std::vector<std::vector<CryptoNote::BlockDetails>>& blocks)
{
	Q_CHECK_PTR(m_node);
	return m_node->doGetBlocks(blockHeights, blocks);
}

bool NodeAdapter::doGetBlocks(const std::vector<Crypto::Hash>& blockHashes, std::vector<CryptoNote::BlockDetails>& blocks)
{
	Q_CHECK_PTR(m_node);
	return m_node->doGetBlocks(blockHashes, blocks);
}

bool NodeAdapter::doGetBlocks(uint64_t timestampBegin, uint64_t timestampEnd, uint32_t blocksNumberLimit, std::vector<CryptoNote::BlockDetails>& blocks, uint32_t& blocksNumberWithinTimestamps)
{
	Q_CHECK_PTR(m_node);
	return m_node->doGetBlocks(timestampBegin, timestampEnd, blocksNumberLimit, blocks, blocksNumberWithinTimestamps);
}
#endif

#if SQ_GET_POOL_TEST
bool NodeAdapter::GetPoolTransactions(std::vector<TransactionDetails>& transactions)
{
	Q_CHECK_PTR(m_node);
	return m_node->GetPoolTransactions(transactions);
}
#endif

#if SQ_GET_TRANSACTION
bool NodeAdapter::doGetTransactions(const std::vector<Crypto::Hash>& transactionHashes, std::vector<TransactionDetails>& transactions)
{
	Q_CHECK_PTR(m_node);
	return m_node->doGetTransactions(transactionHashes, transactions);
}
#endif

#if SQ_ADD_PEER_FRAME
bool NodeAdapter::GetPeerList(std::list<PeerlistEntry>& pl_wite, std::list<PeerlistEntry>& pl_gray)
{
	Q_CHECK_PTR(m_node);
	return m_node->GetPeerList(pl_wite, pl_gray);
}
#endif
...
QDateTime NodeAdapter::getLastBlockTimestamp() const {
	Q_CHECK_PTR(m_node);
	return QDateTime::fromTime_t(m_node->getLastLocalBlockTimestamp());
}
