...
void WalletAdapter::convertWallet(const QString& _filepath, const QString& _password) {
	Q_ASSERT(m_wallet == nullptr);
	Settings::instance().setEncrypted(!_password.isEmpty());
	Q_EMIT walletStateChangedSignal(tr("Converting wallet"));


	m_wallet_old = NodeAdapter::instance().createWallet();
	m_wallet_old->addObserver(this);
	m_wallet_old->m_wallet_version = 1;

	if (QFile::exists(m_walletfile_old)) {
		if (openFile(m_walletfile_old, true)){
			try {
				m_wallet_old->initAndLoad(m_file, _password.toStdString(), 1);
			}
			catch (std::system_error&) {
				closeFile();
				delete m_wallet_old;
				m_wallet_old = nullptr;
			}
		}
	}
	else {
	...
	m_wallet->initAndGenerate(_password.toStdString(), OldKeys);
	if (save(_filepath, true, false))
	{
...
void WalletAdapter::setOldWalletFile(const QString& _path) {
	m_walletfile_old = _path;
}
...
#if SQ_TEST_WALLET_TIME
  const QDateTime bTime = NodeAdapter::instance().getLastBlockTimestamp();
  Q_EMIT walletStateChangedSignal(QString(tr("Wallet synchronized. Height: %1  |  Date: %2%3")).
	  arg(NodeAdapter::instance().getLastLocalBlockHeight()).
	  arg(bTime.isNull() || !bTime.isValid() ? "-" : bTime.toString("yyyy-MM-dd HH:mm::ss")).
	  //arg(QLocale(QLocale::QLocale()).toString(bTime, "dd MMM yyyy, HH:mm:ss")).
	  arg(warningString));
#else
...
#if SQ_ADD_BM_WALLET
bool WalletAdapter::getTransaction(CryptoNote::TransactionId _groupId, CryptoNote::TransactionId _id, CryptoNote::WalletLegacyTransaction& _transaction)
{
	Q_CHECK_PTR(m_wallet);
	try {
		return m_wallet->getTransaction(_groupId, _id, _transaction);
	}
	catch (std::system_error&) {
	}
...
bool WalletAdapter::getGroupList(std::vector<CryptoNote::AddressGroup>& _grouplist)
{
	Q_CHECK_PTR(m_wallet);
	try {
		return m_wallet->getGroupList(_grouplist);
	}
	catch (std::system_error&) {
	}
...
quint64 WalletAdapter::getTransactionCount(CryptoNote::TransactionId _groupId) const
{
	Q_CHECK_PTR(m_wallet);
	try {
		return m_wallet->getTransactionCount(_groupId);
	}
	catch (std::system_error&) {
	}
...
bool WalletAdapter::addBMGroup(const std::string& strAddress, const std::string& strViewkey, const std::string& strSpendkey, std::string strName)
{
	Q_CHECK_PTR(m_wallet);
	int err = 0;
	try
	{
		if (m_wallet->addBMGroup(strAddress, strViewkey, strSpendkey, strName, err))
		{
			Q_EMIT addMessageGroupSignal();
		}
		else
			return false;
	}
...
bool WalletAdapter::deleteBMGroup(const std::string& strAddress)
{
	Q_CHECK_PTR(m_wallet);
	int err = 0;
	try
	{
		if (m_wallet->deleteBMGroup(strAddress, err))
		{
			Q_EMIT deleteMessageGroupSignal();
			return true;
		}
		else
			return false;
...
bool WalletAdapter::createGroup(std::string& str_add, std::string& str_vkey, std::string& str_skey)
{
	Q_CHECK_PTR(m_wallet);
	AccountKeys mkeys;
...
int WalletAdapter::getGroupIdById(const int& _rowId)
{
	Q_CHECK_PTR(m_wallet);
	return m_wallet->getGroupIdById(_rowId);
}
#endif

#if SQ_ADD_BM_SEGMENT
void WalletAdapter::sendSegment(int& _size)
{
	if (_size < 1)
		return;
	Q_CHECK_PTR(m_wallet);
	try {
		...
		m_wallet->sendSegment(_size, txId);
		if ((_size > 0) && (txId != std::numeric_limits<TransactionId>::max()))
		{
			m_sentTransactionId = txId;
			Q_EMIT walletStateChangedSignal(tr("Sending transaction"));
		}
	}
	catch (std::system_error&) {
